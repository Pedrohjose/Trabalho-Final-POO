...
public class Produto implements GerenciaCSV {
    ...
    public String toCSV() {
        ...
        // Monta o bloco com o ; no final de cada linha
    ...
    // Método estático que lê a lista de linhas do DAO
    public static Produto fromCSV(List<String> linhasDoObjeto) {
        if (linhasDoObjeto.size() < 5) {
            throw new IllegalArgumentException("Dados insuficientes para criar Produto. Linhas lidas: " + linhasDoObjeto.size());
        }
        
        try {
            // 1. Separa a chave do valor usando " : "
            ...
            // 2. Remove o ";" final usando substring e converte
            ...
            CategoriasProdutos categoria = CategoriasProdutos.valueOf(valorLinha4.substring(0, valorLinha4.length() - 1));
            
            return new Produto(codigo, nome, preco, quantidade, categoria);
            
        } catch (Exception e) {
            throw new IllegalArgumentException("Erro ao formatar dados do produto. Verifique o arquivo CSV. Erro: " + e.getMessage());
        }
    }
